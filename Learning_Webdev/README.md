# PROJECT: MIKE AND DAN FITNESS WEBSITE
...
As this website uses nodejs for the backend and to localhost it requires it to be installed to work. Use the below commands if on Ubuntu
```bash
sudo apt update && sudo apt install nodejs npm -y)
```

As this website uses sqlite3 for the database it also requires that to be installed.
```bash
sudo apt update && sudo apt install sqlite3
```

As this website uses bcrypt it is a requirement that that is installed as well.
```bash       
npm install bcrypt.
```
